...
from touchdown_time_reward import (
    LARGE_RECORD_MAX_BONUS,
    MEDIAN_SLOW_MAX_PENALTY,
    TouchdownTimeRewardTracker,
    WARMUP_SUCCESSES,
)
...
def _build_agent2_env(
    ...
    label: str,
    touchdown_time_state_path: Path,
):
    ...
    wrapped = RpcDominantAgent2RewardEnv(
        parallel,
        touchdown_time_tracker=TouchdownTimeRewardTracker(
            touchdown_time_state_path
        ),
    )
...
def _create_new_run(requested_stage: str) -> tuple[Path, dict[str, Any]]:
    ...
    state = {
        ...
        "reward_contract": {
            ...
            * AGENT2_IDENTITY_REWARD_SCALE,
            "touchdown_time_warmup_successes": WARMUP_SUCCESSES,
            "touchdown_time_max_bonus": LARGE_RECORD_MAX_BONUS,
            "touchdown_time_max_penalty": MEDIAN_SLOW_MAX_PENALTY,
        },
        ...
        "terminal_controller_deterministic": False,
        "terminal_exploration_steps": 50,
        "terminal_z_controller_mode": "range_governed_conditional_climb",
        "touchdown_time_state_file": str(run_root / "touchdown_time_reward_state.json"),
...
def _load_or_create_run(requested_stage: str) -> tuple[Path, dict[str, Any]]:
    ...
    state["version"] = "COOPERATIVE_FINAL_ROLLBACK3_RANGE_Z_V3"
    state["terminal_controller_deterministic"] = False
    state["terminal_exploration_steps"] = 50
    state["terminal_z_controller_mode"] = "range_governed_conditional_climb"
    state["touchdown_time_state_file"] = str(
        run_root / "touchdown_time_reward_state.json"
    )
    reward_contract = dict(state.get("reward_contract", {}) or {})
    reward_contract.update(
        {
            "touchdown_time_warmup_successes": WARMUP_SUCCESSES,
            "touchdown_time_max_bonus": LARGE_RECORD_MAX_BONUS,
            "touchdown_time_max_penalty": MEDIAN_SLOW_MAX_PENALTY,
    ...
    state["reward_contract"] = reward_contract
    atomic_json(state_path, state)
...
def _run_phase(
    ...
    touchdown_time_state_path = run_root / "touchdown_time_reward_state.json"

    if role == ROLE_AGENT2:
        env = _build_agent2_env(
            agent1_checkpoint, identity, phase_id, touchdown_time_state_path
        )
    ...
    try:
        ...
        print("[COOP FINAL] Terminal phase   : 50 range-governed steps; conditional Agent-2 climb")
        print("[COOP FINAL] Time curriculum  : 20 successful-touchdown warmup")
        print("=" * 100)
...
def _publish_phase_pair(
    ...
    pair_state = {
        ...
        "terminal_exploration_steps": 50,
        "terminal_z_controller_mode": "range_governed_conditional_climb",
        "touchdown_time_state": (
            _read_json(run_root / "touchdown_time_reward_state.json")
            if (run_root / "touchdown_time_reward_state.json").is_file()
            else None
        ),
    }

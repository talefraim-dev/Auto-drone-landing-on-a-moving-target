...
class PhaseHealthCallback(BaseCallback):
    ...
    def __init__(
        ...
        self.touchdown_times_s: list[float] = []
        self.touchdown_time_rewards: list[float] = []
        self.touchdown_time_classes: Counter[str] = Counter()
    ...
    def _record_terminal(self, info: dict[str, Any], reward: float) -> None:
        ...
        terminal_violation = bool(
            handoff
            and bool(info.get("policy_actions_suppressed_after_terminal_handoff", False))
        )
        ...
        if terminal_violation:
            ...
            raise RuntimeError(
                "Safety violation: policy actions were suppressed during the controlled terminal window."
            )
        ...
        touchdown_time = self._finite_float(info.get("touchdown_time_s"))
        if touchdown_time is not None:
            self.touchdown_times_s.append(touchdown_time)
        time_reward = self._finite_float(info.get("touchdown_time_reward"), 0.0)
        if time_reward is not None:
            self.touchdown_time_rewards.append(time_reward)
        time_class = str(
            info.get("touchdown_time_classification", "UNREPORTED") or "UNREPORTED"
        )
        self.touchdown_time_classes[time_class] += 1
        self.recent.append(
            {
                "success": success,
    ...
    def summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "mean_touchdown_time_s": (
                float(np.mean(self.touchdown_times_s)) if self.touchdown_times_s else None
            ),
            "median_touchdown_time_s": (
                float(np.median(self.touchdown_times_s)) if self.touchdown_times_s else None
            ),
            "mean_touchdown_time_reward": (
                float(np.mean(self.touchdown_time_rewards))
                if self.touchdown_time_rewards
                else 0.0
            ),
            "touchdown_time_classifications": dict(self.touchdown_time_classes),

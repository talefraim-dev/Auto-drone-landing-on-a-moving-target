...
class Agent1P2Env(gym.Env):
    ...
    def _execute_parallel_command(self, agent2_vz_mps: float) -> dict[str, Any]:
        ...
        bottom_live = bool(bottom_guidance.get("live_match", False))
        ...
        agent1_xy_weight = float(
            self.bottom_live_agent1_xy_weight if bottom_live else 1.0
        )
        bottom_vx = (
        ...
        return {
            ...
            "bottom_guidance_live": bool(
                bottom_guidance.get("live_match", False)
            ),
            "bottom_xy_authority": (
                "BOTTOM_PREDICTIVE"
                if bool(bottom_guidance.get("live_match", False))

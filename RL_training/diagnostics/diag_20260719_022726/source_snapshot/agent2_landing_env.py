...
class Agent2Config:
    ...
    target_velocity_ema_alpha: float = 0.55
    ...
    # Predictive bottom-camera guidance. The controller estimates the target's
    # image-space position at t+1 using the filtered relative image velocity,
    # then corrects toward that future position while the actor-velocity feed-
    # forward carries the drone at the platform's world/body-frame speed.
    ...
    predictive_bottom_normal_correction_max_mps: float = 1.10
    predictive_bottom_catchup_correction_max_mps: float = 2.20
    predictive_bottom_touchdown_catchup_max_mps: float = 1.40
...
class Agent2LandingEnv(gym.Env):
    ...
    def _compute_predictive_bottom_guidance(
        ...
        *,
        ...
        """Predict the bottom-frame target position at t+1 and correct toward it.

        The image velocity is relative motion: it already contains the combined
        effect of target motion, drone motion and camera geometry. World/body
        target velocity is handled separately as feed-forward, so this method
        produces only the remaining relative-position correction.
        """
        ...
        if not live_match or not bool(self.cfg.predictive_bottom_enabled):
        ...
        predicted_err_x = float(np.clip(err_x + vel_x * horizon, -1.50, 1.50))
        predicted_err_y = float(np.clip(err_y + vel_y * horizon, -1.50, 1.50))
        ...
        catchup_active = bool(self._predictive_catchup_active)
        release_streak = int(self._predictive_catchup_release_streak)
        if update_hysteresis:
            if catchup_active:
                if exit_catchup:
                    release_streak += 1
        ...
        result = {
            ...
            # A LIVE bottom view is authoritative for XY during landing.
            "bottom_blend_strength": 1.0,
            "center_error": center_error,
            "bbox_area_norm": area,
            "relative_height_m": height,
    ...
    def _all_reference_embeddings(self) -> list[torch.Tensor]:
        ...
        references.extend(self._adaptive_embeddings)
        return references
    ...
    def _maybe_update_adaptive_embedding_bank(
        ...
        if int(self._step) - int(self._last_adaptive_embedding_update_step) < interval:
            return 0
        ...
        if additions > 0:
            self._last_adaptive_embedding_update_step = int(self._step)
            self._adaptive_embedding_updates += 1
        return additions
    ...
    def _vertical_control_state(self, info: dict[str, Any]) -> tuple[str, bool, str]:
        """Fast-acquire landing lock with gap tolerance and safe Z pauses.

        LIVE alignment acquires the lock. A brief PRED/no-detection gap blocks
        descent immediately but preserves the lock for reacquisition. The lock
        is released only after a sustained visual gap or several consecutive
        badly aligned LIVE frames.
        """
        live_match = bool(info.get("bottom_match_live", False))
        similarity = float(info.get("bottom_similarity", 0.0))
        center_error = float(info.get("bottom_center_error", 999.0))
        ...
        if bool(self._predictive_catchup_active):
            self._landing_lock_bad_live_steps = 0
            ...
            return (
                ...
                False,
                "predicted_t_plus_1_position_or_relative_velocity_unsafe",
            )
        ...
        if self._alignment_ready_streak >= required:
            ...
            self._landing_lock_acquired_step = int(self._step)
            self._landing_lock_visual_gap_steps = 0
            self._landing_lock_bad_live_steps = 0

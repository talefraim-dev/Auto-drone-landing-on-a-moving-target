...
// DynamicZoomMarker component remains exactly the same as your original code
const DynamicZoomMarker = ({ lat, lng, markerColor, isConnected }) => {
    const [currentZoom, setCurrentZoom] = useState(14);
    useMapEvents({ zoomend: (e) => setCurrentZoom(e.target.getZoom()) });
    ...
    return (
        <CircleMarker center={[lat, lng]} pathOptions={{ color: markerColor, fillColor: markerColor, fillOpacity: 0.6 }} radius={calculateRadius()}>
            <Popup>{isConnected ? `UAV Position: ${lat.toFixed(4)}, ${lng.toFixed(4)}` : 'No Connection'}</Popup>
        </CircleMarker>
    );
};
...
const Minimap = ({ lat, lng, isConnected }) => {
  ...
  const markerColor = isConnected ? '#00f0ff' : '#ef4444';

  return (
    <MapContainer 
        center={[defaultLat, defaultLng]} 
        zoom={14} minZoom={12} maxZoom={16} maxBounds={mapBounds}

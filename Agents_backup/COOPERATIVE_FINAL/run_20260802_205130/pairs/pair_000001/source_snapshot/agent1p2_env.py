...
class Agent1P2Env(gym.Env):
    ...
    def _execute_parallel_command(self, agent2_vz_mps: float) -> dict[str, Any]:
        ...
        if terminal_contact_hold:
            # At the deterministic 0.35 m handoff, the last safe XY snapshot is
            # frozen. Agent 1 still executes for state continuity, but it is not
            # allowed to introduce new XY or yaw commands.
            agent1_action = np.asarray(agent1_action, dtype=np.float32).copy()
            agent1_action[0] = 0.0
            agent1_action[1] = 0.0
            agent1_action[3] = 0.0
        ...
        if terminal_contact_hold:
            ...
            bottom_vx = 0.0
            bottom_vy = 0.0
            agent1_xy_weight = 0.0
            bottom_guidance_active = False
            bottom_live = False
            bottom_prediction_only = False
        ...
        self.agent1_env.set_parallel_control_overrides(
            ...
            terminal_contact_hold=terminal_contact_hold,
        )

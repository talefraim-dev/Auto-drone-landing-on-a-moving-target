...
class TrainAgent1WithFrozenAgent2Env(gym.Env):
    ...
    def __init__(
        ...
        device: str = "auto",
        collapse_terminal_rollout: bool = True,
    ) -> None:
        super().__init__()
        self.parallel_env = parallel_env
        self.collapse_terminal_rollout = bool(collapse_terminal_rollout)
    ...
    def _complete_deterministic_terminal_rollout(
        self,
        info: dict[str, Any],
        terminated: bool,
        truncated: bool,
    ) -> tuple[bool, bool, dict[str, Any]]:
        """Finish the latched terminal controller without collecting fake policy actions.

        Once range-terminal handoff is active, Agent 1 and Agent 2 actions are
        physically overridden. Repeated PPO transitions after that point would
        assign contact reward to actions that were never executed. Instead, the
        deterministic terminal sequence is rolled internally and its final
        result is credited to the one real policy transition that entered it.
        """
        if (
            not self.collapse_terminal_rollout
            or bool(terminated or truncated)
            or not bool(info.get("range_terminal_handoff_active", False))
        ):
            return bool(terminated), bool(truncated), info

        zero_agent1 = np.zeros(self.action_space.shape, dtype=np.float32)
        zero_agent2 = np.zeros(self.parallel_env.action_space.shape, dtype=np.float32)
        steps = 0
        last_info = dict(info)
        while steps < TERMINAL_ROLLOUT_MAX_STEPS:
            self._pending_agent1_action = zero_agent1
            next_obs, _ignored_reward, terminated, truncated, next_info = (
                self.parallel_env.step(zero_agent2)
            )
            self._agent2_obs = np.asarray(next_obs, dtype=np.float32)
            last_info = dict(next_info or {})
            steps += 1
        ...
        if not bool(terminated or truncated):
            raise RuntimeError(
                "Deterministic terminal controller did not finish within "
                f"{TERMINAL_ROLLOUT_MAX_STEPS} internal steps."
            )

        last_info.update(
            {
                "terminal_controller_rollout_steps": int(steps),
                "terminal_result_credited_to_handoff_action": True,
                "policy_actions_suppressed_after_terminal_handoff": True,
            }
        )
        return bool(terminated), bool(truncated), last_info
    ...
    def _step_landing_phase(self, action):
        ...
        # Preserve the reward caused by the real Agent-1 action. Internal
        # terminal-controller steps intentionally use zero/overridden actions
        # and must not replace this transition's dense learning signal.
        ...
        info = dict(info or {})
        terminated, truncated, info = self._complete_deterministic_terminal_rollout(
...
class RpcDominantAgent2RewardEnv(gym.Wrapper):
    ...
    def __init__(self, env, collapse_terminal_rollout: bool = True):
        super().__init__(env)
        self.collapse_terminal_rollout = bool(collapse_terminal_rollout)
        self._identity_reward = IdentityStabilityReward()
        self._dense_episode_total = 0.0
    ...
    def _complete_deterministic_terminal_rollout(
        ...
        terminated: bool,
        truncated: bool,
    ) -> tuple[np.ndarray | None, bool, bool, dict[str, Any]]:
        if (
            not self.collapse_terminal_rollout
            or bool(terminated or truncated)
            or not bool(info.get("range_terminal_handoff_active", False))
        ):
            return None, bool(terminated), bool(truncated), info
        ...
        steps = 0
        while steps < TERMINAL_ROLLOUT_MAX_STEPS:
            next_obs, _ignored_reward, terminated, truncated, next_info = self.env.step(zero_action)
            final_obs = np.asarray(next_obs, dtype=np.float32)
            last_info = dict(next_info or {})
            steps += 1
            if bool(terminated or truncated):
                break
        ...
        return final_obs, bool(terminated), bool(truncated), last_info
    ...
    def step(self, action):
        ...
        final_obs, terminated, truncated, info = self._complete_deterministic_terminal_rollout(
            info, bool(terminated), bool(truncated)
        )
        ...
        reward = float(
            dense + identity_reward - descending_scale_jump_penalty + shared_terminal
        )
        info.update(identity_diag)
        info.update(
            {
